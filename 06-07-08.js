...
function crearClasePersona() {
  class Persona {
    ...
    getFriends() {
      ...
      // Tu código aca:
      return this.amigos.map(persona => persona.nombre);
    }
    ...
    getPromedioEdad() {
      ...
      if (this.amigos.length === 0) {
        return 0; //en caso de que no tenga amigos para evitar una divición por "0"
     }
      var sumaedades = 0
      var promedio = 0
      for (var i = 0; i < this.amigos.length; i ++){
      sumaedades = sumaedades + this.amigos[i].edad;
     };

     promedio = sumaedades / this.amigos.length;
     return promedio;
    }
